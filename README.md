...
为了适配不同页数的PDF（如2页标准报告、3页扩展报告），锚点搜索采用三级fallback策略：

1. **优先搜索最后一页**：ESI服务报告的签名框通常在最后一页，优先检查可提高匹配成功率和性能
2. **尝试指定页面**：如果最后一页未找到，检查配置中指定的 `page_index` 页（向后兼容）
3. **遍历所有页面**：如果前两步都失败，遍历所有剩余页面作为最后的fallback

这个策略确保：
- ✅ 2页PDF（H5R30, H5R43, H5R54, H5R12）签名正确放置在第2页
- ✅ 3页PDF（H5R5）签名正确放置在第3页
- ✅ 向后兼容现有配置
- ✅ 自动适配未来可能出现的其他页数变体

**关键行为**：找到锚点后，签名会叠加在**实际找到锚点的那一页**上，而不是配置文件中指定的 `page_index`。`page_index` 仅作为搜索提示和范围验证参数（若超出PDF总页数会提前报错）。
...
**实际行为**：找到锚点后，签名会叠加在**实际找到锚点的页面**上，而不是配置文件中指定的 `page_index`。`page_index` 仅作为搜索提示和验证参数（若超出范围会提前报错）。
